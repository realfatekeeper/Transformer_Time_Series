...
#python data_prep.py --data-path C:\Users\triwi\ConvTransformerTimeSeries-master\first_trial.csv --save-name elect --window-size 100 --stride-size 500 --test-ratio 0.3
# & C:/Users/Acer/anaconda3/envs/pytorch/python.exe c:/Users/Acer/Documents/GitHub/Transformer_Time_Series/data_prep.py --data-path C:\Users\Acer\Documents\GitHub\Transformer_Time_Series\first_trial.csv --save-name elect --window-size 100 --stride-size 500 --test-ratio 0.3
...
def save_dataset(save_path,save_name,xtrain_input,v_input_train,label_train,timeseries_dict_train,transformation_covariates_var, transformation_covariates,
				xval_input, label_val, v_input_val, timeseries_dict_val,
				xtest_input,v_input_test,label_test,timeseries_dict_test, split_dates):
	'''
	...
	# saving validation
	prefix = os.path.join(save_path, 'val_')

	np.save(prefix+'data_'+save_name, xval_input)
	np.save(prefix+'v_'+save_name, v_input_val)
	np.save(prefix+'label_'+save_name, label_val)
	np.save(prefix+'timeseries_dict_'+save_name, timeseries_dict_val)
	...
	meta_information['validation_start'] = str(validation_start)
	meta_information['validation_end'] = str(validation_end)
...
def main(data_path,data_covariates_path,date_column,cast_float16,window_size,stride_size,test_ratio,save_name):
	...
	train_end = Table.index[int(l_total*(1-test_ratio)*0.8)]

	freq = pd.infer_freq(Table.index)
	
	freq = to_offset(freq)
	
	validation_start = Table.index[int(l_total*(1-test_ratio)*0.8)+1]
	validation_end = Table.index[int(l_total*(1-test_ratio))]
	...
	validation_data = Table[validation_start:validation_end].values
	test_data = Table[test_start:test_end].values

	print('\t #train_data index range:',train_start,train_end)
	print('\t #validation_data index range:',validation_start,validation_end)
	...
	if not covariates_var is None : 
		...
		validation_covariates_var = []
		for var in covariates_var :
			validation_covariates_var.append(var[validation_start:validation_end].values[:,:,None])
		validation_covariates_var = np.concatenate(validation_covariates_var,axis=2)
		...
		validation_covariates_var = None
	...
	covariates_validation = gen_covariates(Table[validation_start:validation_end].index)
	...
	data_start_validation = (validation_data!=0).argmax(axis=0) #find first nonzero value in each time series
	...
	xval_input, label_val, v_input_val, timeseries_dict_val, _, _ = prep_data(validation_data, window_size=window_size_train, stride_size=stride_size, data_start=data_start_validation, timeseries_list=timeseries_list, covariates=covariates_validation, covariates_var=validation_covariates_var,transformation_covariates_used = transformation_covariates, transformation_covariates_var_used = transformation_covariates_var)
	...
	save_dataset(save_path,save_name,xtrain_input,v_input_train,label_train,timeseries_dict_train,transformation_covariates_var, transformation_covariates,
				xval_input, label_val, v_input_val, timeseries_dict_val,
				xtest_input,v_input_test,label_test,timeseries_dict_test, 
